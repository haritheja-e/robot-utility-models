...
from utils import AveragingDict, set_seed_everywhere, Callbacks
...
class Workspace:
    def __init__(self, cfg: OmegaConf):
        ...
        self.files_list_path = str(cfg.files_list_path)
            ...
            if (
                ...
                if freeze := cfg.get("freeze_submodule"):
                    ...
                    if freeze.get("loss_fn"):
                        logger.info("Freezing the loss fn")
    ...
    def _setup_dataloaders(
        ...
        (self.work_dir / self.files_list_path).mkdir(parents=True, exist_ok=True)
    ...
    def to_device(self, manyTensors: Iterable[torch.Tensor]):
        ...
        # TODO Hardcoded conversion assuming the first (and only the first) tensor is the image.
    ...
    def _train_epoch(self):
        ...
        # TODO: this fixes the scheduling bug not sure how it will affect other scheduling
    ...
    def _train_step(self, batch, overall_loss_dict, iterator):
        ...
        loss, loss_dict = self.loss_fn(
            ...
            model_outputs,
            second_half=(
                self._epoch > self._cfg.get("train_freeze_ratio", 1) * self._cfg.num_epochs
            ),
        )
    ...
    def save_model(self):
        ...
        try:
            with open(self.work_dir / self.model_path / "description.txt", "x") as file:
                file.write(self._cfg.description)
    ...
    def resume_training(self):
        ...
        self.load_optimizer_and_scheduler(checkpoint)
...
class VQBeTWorkspace(Workspace):
    def _setup_opt_and_sched(self):
        # Set up optimizer.
        if self.loss_fn._vqbet.sequentially_select:
            self._params1 = chain(
                self.model.parameters(),
                self.loss_fn._obs_adapter.parameters(),
                self.loss_fn._vqbet._gpt_model.parameters(),
                self.loss_fn._vqbet._map_to_cbet_preds_bin1.parameters(),
                self.loss_fn._vqbet._map_to_cbet_preds_bin2.parameters(),
            )
        else:
            self._params1 = chain(
                self.model.parameters(),
                self.loss_fn._obs_adapter.parameters(),
                self.loss_fn._vqbet._gpt_model.parameters(),
                self.loss_fn._vqbet._map_to_cbet_preds_bin.parameters(),
            )
        self._params2 = chain(
            self.loss_fn._vqbet._map_to_cbet_preds_offset.parameters()
        )

        # group 1: trained during only first half
        # group 2: trained through first half and second half.

        self.optimizer: Dict[str, torch.optim.Optimizer] = {
            "group1": hydra.utils.instantiate(
                self._cfg.optimizer,
                params=self._params1,
                lr=self._scaled_lr,
            ),
            "group2": hydra.utils.instantiate(
                self._cfg.optimizer,
                params=self._params2,
                lr=self._scaled_lr,
            ),
        }
        # Set up scheduler.
        self.scheduler: Dict[str, torch.optim.lr_scheduler.LRScheduler] = {
            "group1": hydra.utils.instantiate(
                self._cfg.scheduler, optimizer=self.optimizer["group1"]
            ),
            "group2": hydra.utils.instantiate(
                self._cfg.scheduler, optimizer=self.optimizer["group2"]
            ),
        }
        self._norm_clip = self._cfg.get("clip_gradient_norm", None)
    ...
    def _train_epoch(self):
        ...
        # TODO: this fixes the scheduling bug not sure how it will affect other scheduling
        if not accelerator.optimizer_step_was_skipped:
            self.scheduler["group1"].step(self._epoch)
            self.scheduler["group2"].step(self._epoch)
        self._total_loss /= len(self._train_dataloaders)
        overall_loss_dict.update({"total_loss": self._total_loss})
        if accelerator.is_local_main_process:
            logger.info(f"Train loss: {str(overall_loss_dict)}")
        epoch_summary = overall_loss_dict.full_summary
        epoch_summary.update(
            {"_epoch": self._epoch, "lr": self.scheduler["group2"].get_last_lr()[0]}
        )
        accelerator.log(epoch_summary)

    def _train_step(self, batch, overall_loss_dict, iterator):
        callback.begin_batch()
        if self._epoch < self._cfg.get("train_freeze_ratio", 1) * self._cfg.num_epochs:
            self.optimizer["group1"].zero_grad(set_to_none=True)
            self.optimizer["group2"].zero_grad(set_to_none=True)
        else:
            self.optimizer["group2"].zero_grad(set_to_none=True)
        batch_in_device = self.to_device(batch)
        model_outputs = self.model(batch_in_device)
        ...
        accelerator.backward(loss)
        if self._norm_clip and accelerator.sync_gradients:
            accelerator.clip_grad_norm_(self._params1, self._norm_clip)
            accelerator.clip_grad_norm_(self._params2, self._norm_clip)
        if self._epoch < self._cfg.get("train_freeze_ratio", 1) * self._cfg.num_epochs:
            self.optimizer["group1"].step()
            self.optimizer["group2"].step()
        else:
            self.optimizer["group2"].step()
        self._total_loss += loss.cpu().item()
        overall_loss_dict.update(loss_dict)
    ...
    def save_model(self):
        ...
        (self.work_dir / self.model_path).mkdir(parents=True, exist_ok=True)
        unwrapped_model = accelerator.unwrap_model(self.model)
        unwrapped_optimizer1 = accelerator.unwrap_model(self.optimizer["group1"])
        unwrapped_optimizer2 = accelerator.unwrap_model(self.optimizer["group2"])
        unwrapped_scheduler1 = accelerator.unwrap_model(self.scheduler["group1"])
        unwrapped_scheduler2 = accelerator.unwrap_model(self.scheduler["group2"])
        unwrapped_loss_fn = accelerator.unwrap_model(self.loss_fn)
        accelerator.save(
            {
                "cfg": OmegaConf.to_container(self._cfg, resolve=True),
                "epoch": self._epoch,
                "model": unwrapped_model.state_dict(),
                "optimizer1": unwrapped_optimizer1.state_dict(),
                "optimizer2": unwrapped_optimizer2.state_dict(),
                "scheduler1": unwrapped_scheduler1.state_dict(),
                "scheduler2": unwrapped_scheduler2.state_dict(),
                "loss_fn": unwrapped_loss_fn.state_dict(),
            },
            self.work_dir / self.model_path / "checkpoint.pt",
        )
    ...
    def load_optimizer_and_scheduler(self, checkpoint):
        if "optimizer1" in checkpoint and "optimizer2" in checkpoint:
            self.optimizer["group1"].load_state_dict(checkpoint["optimizer1"])
            self.optimizer["group2"].load_state_dict(checkpoint["optimizer2"])
        else:
            logger.warning("No optimizer found in checkpoint. Using default optimizer.")

        if "scheduler1" in checkpoint and "scheduler2" in checkpoint:
            self.scheduler["group1"].load_state_dict(checkpoint["scheduler1"])
            self.scheduler["group2"].load_state_dict(checkpoint["scheduler2"])
        else:
            logger.warning("No scheduler found in checkpoint. Using default scheduler.")



class DiffusionPolicyWorkspace(Workspace):
    def _train_epoch(self):
        self.model.train()
        self.loss_fn.train()
    ...
    def _eval_step(self, batch, overall_loss_dict, iterator):
        ...
        batch_in_device = self.to_device(batch)
        model_outputs = self.model(batch_in_device)
        loss, loss_dict = self.loss_fn(batch_in_device, model_outputs, eval=True)
        self._total_loss += loss.cpu().item()
        overall_loss_dict.update(loss_dict)
        iterator.set_postfix(overall_loss_dict.summary)
        accelerator.log(overall_loss_dict.full_summary)
...
@hydra.main(config_path="configs", config_name="train_vqbet.yaml", version_base="1.2")
def main(cfg: OmegaConf):
    ...
    elif "vqbet" in loss_fn_str:
        workspace = VQBeTWorkspace(cfg) if cfg.split_training else Workspace(cfg)
    elif "diffusion_policy" in loss_fn_str:
