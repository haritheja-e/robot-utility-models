...
class DataLoaderConfig:
    ...
    def __init__(
        ...
        self._use_language_labels = language_label_file is not None and os.path.exists(
            language_label_file
        )
        if self._use_language_labels:
            lang_label_object = np.load(language_label_file, allow_pickle=True)
    ...
    def _build_trajectories(self):
        ...
        for root_path in tqdm.tqdm(
            ...
            # Use language label file (optionally) to weed out invalid trajectories.
            if self._use_language_labels:
                if (task, home, env) not in self._language_labels:
                    continue
                self._traj_lang_label.append(self._language_labels[(task, home, env)])
    ...
    def _subsample_trajectories(self):
        ...
        if self._use_language_labels:
            self._traj_lang_label = [self._traj_lang_label[i] for i in chosen_indices]

    def get_language_labels(self):
        if self._use_language_labels:
            return self._traj_lang_label
        else:
            return None
